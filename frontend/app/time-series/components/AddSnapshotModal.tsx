  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const snapshot = {
        ...
        date,
        previewUrl: `${apiBase}/data/results/${result.id}/preview.png`,
        mean: result.meta?.stats?.mean || result.raster?.mean || 0,
        min: result.meta?.stats?.min || result.raster?.min || 0,
        max: result.meta?.stats?.max || result.raster?.max || 0,
      };
    ...
    <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl p-8 max-w-2xl w-full shadow-2xl">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">Добавить снимок</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-900 text-2xl">×</button>
        </div>

        <div className="space-y-6">
          <div>
            <label className="block text-sm text-gray-700 mb-2">Дата снимка</label>
            <input
              type="date"
              value={date}
              onChange={(e) => setDate(e.target.value)}
              className="w-full p-3 rounded-xl bg-white border border-gray-300 text-gray-900"
            />
          </div>

          <div>
            <label className="block text-sm text-gray-700 mb-2">Тип загрузки</label>
          ...
          {uploadType === "single" ? (
            <div>
              <label className="block text-sm text-gray-700 mb-2">Файл снимка</label>
              <div
                onDrop={handleDrop}
            ...
            <div>
              <label className="block text-sm text-gray-700 mb-2">Объединение каналов</label>
              <button
                onClick={() => setShowBandStacking(true)}
                className="w-full p-8 rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50 hover:bg-gray-100 transition"
              >
        ...
        <div className="mt-8 flex gap-3">
          <button
            onClick={onClose}
            disabled={loading}
            className="flex-1 p-3 rounded-xl bg-gray-100 text-gray-900 hover:bg-gray-200 transition disabled:opacity-50"
          >
            Отмена
          </button>
          <button
            ...
            className="flex-1 p-3 rounded-xl bg-emerald-600 text-white font-semibold hover:bg-emerald-700 transition disabled:opacity-50"
          >
